...
int rr(char* filename, int verbose)
{
	...
	while (number_of_terminated < process_list.size())
	{
		if (verbose == 1)
		{
			...
			for (int i = 0; i < process_list.size(); ++i)
			{
				...
				if (process_list[i].current_status == "blocked")
					...
					pre_cycle_string += int_2_string(quantum_police);
				else
					pre_cycle_string += "0";
				pre_cycle_string += "		";
			}
		...
		if (rp_num == -1)
		{
			...
			{	//take the first process directly from the main vector to running 
				...
				if (process_list[rp_num].time_left_in_current_CPU_burst >= quantum_num)
					quantum_police = quantum_num;
				else
					quantum_police = process_list[rp_num].time_left_in_current_CPU_burst;
		...
		else
		{
			if (process_list[rp_num].current_status == "running") //it will most definitely always be
			{
				if (quantum_police < 0)
					break;
				if (process_list[rp_num].time_left_in_current_CPU_burst > 1 && quantum_police > 1)
				...
				else //time left in burst is 1 or 0 or quantum police is 1 or 0
				{
					...
					{	//run then block terminate
						...
						{	//completely done running -> terminate
							...
							if (ready_queue.size() > 0)
							{
								...
								//initialize run
								//do normal initialize if time left in current CPU Burst is == 0
								if (process_list[rp_num].time_left_in_current_CPU_burst == -1)
								{
									int t = randomOS(process_list[rp_num].b, verbose);
									process_list[rp_num].current_status = "running";
								...
								{	//means the process was a preempted process
									process_list[rp_num].current_status = "running";
									if (process_list[rp_num].time_left_in_current_CPU_burst >= quantum_num)
									{
										quantum_police = quantum_num;
					...
					else //burst is > 1 but quantum police is 1 or less
					{	//not terminated, not blocking just preempted
						//run once more then preempt
						process_list[rp_num].total_cpu_time_left -= 1;
						process_list[rp_num].time_left_in_current_CPU_burst -= 1;
						quantum_police -= 1;
						process_list[rp_num].current_status = "ready";
						ready_queue.push_back(process_list[rp_num]);
						//since the running process just preempted, find something to run
						if (ready_queue.size() > 0)
						{
							rp_num = ready_queue[0].process_number;
							ready_queue.erase(ready_queue.begin());
